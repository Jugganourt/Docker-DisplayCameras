...
if [ ! -d $DIR/displaycameras ] 
then
    # Create the displaycameras directory
    mkdir displaycameras
fi

# Move into the newly created directory
cd displaycameras

# Create the displaycameras.conf file
cat > displaycameras.conf << EOF
...
read -p "What is the name of the Camera? "
cameraName="$REPLY"

read -p "What is the steam url? "
cameraStreamUrl="$REPLY"

cat > layout.conf.default << EOF
# Windows
windows=(main)
